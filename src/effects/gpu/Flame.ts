...
export class Flame extends GPUParticleEffect {
  private options: FlameOptions;
...
fn force(v: float3, t: float) -> float3 {
    let sc = custom.Pb*(rot(1.215*time.elapsed)+0.15*rot(5.515*time.elapsed).yx);
    let sc2 = 7.0*rot(1.0*time.elapsed) + 5.0*rot(0.25*1.14519*2.0*time.elapsed);
    let sc3 = 5.0*rot(1.3547*time.elapsed) + 5.0*rot(1.7547*time.elapsed);
    let sc4 = custom.Pc*rot(0.4*time.elapsed + t);
    var f = sin(custom.Pa*vec3(sc3.x + 1.0-t, sc2.x + sc3.y + 2.0+t, sc2.y + 3.0+2.0*t) * v);
    ...
    f += sc.x*sin(8.0*vec3(1.5, 2.0, 1.6) * f.yzx);
    f -= sc.y*sin(8.0*vec3(0.5, 0.2, 2.6) * f.yzx);
    f += sc.y*sin(8.0*vec3(1.5, 2.0, 1.6) * f.xzx);
    f -= sc.x*sin(8.0*vec3(0.5, 0.2, 2.6) * f.yzx);
    return f + vec3(0.0, 0.3, 0.0);
}
...
fn point_gen() {
    ...
    let circle = 0.5*rot(2.0*3.14159*r4.x);
    var p = vec3(circle.x, 0.0,circle.y);
    let sc = custom.Pd*rot(0.5*time.elapsed);
    let delta = 0.0*vec3(0.1+sc.x,0.2,0.4+sc.y)*sin(50.0*vec3(1.1+sc.y,1.0,1.3)*r4.x) - vec3(0.0,0.33,0.0);
    p += delta;
    p += vec3(0.0,0.75,0.0);

    let t = -r4.y;
    var color = mix(vec3(1.0,0.4,0.1), vec3(1.00, 0.40, 0.1), r4.y);
...
fn main_image(@builtin(global_invocation_id) id: uint3) 
{
  ...
  compute({ overallAvg, upperAvgFr, lowerAvgFr, classifyOutput }) {
    ...
    customUniformBuffer.setSubData(
      ...
      new Uint8Array(
        new Float32Array([
          options.radius + modulate(overallAvg, 0, 256, 0, 0.5),
          options.samples,
          ...
          options.pa,
          options.pb,
          (modulate(upperAvgFr, 0, 1, 0.5, 4) / 4) * options.pc,
          (modulate(lowerAvgFr, 0, 1, 0.5, 4) / 4) * options.pd,
          options.pe,
          options.pf,
          options.dt,
        ]).buffer,
  ...
  update(options: Partial<FlameOptions>) {
    this.options = {
      ...this.options,
      ...options,
    };
