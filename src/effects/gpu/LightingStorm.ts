...
fn force(v: float3, t: float) -> float3 {
    let sc = custom.Pc*(rot(3.215*(time.elapsed * 0.03))+0.15*rot(15.515*(time.elapsed * 0.03)).yx);
    let sc2 = 7.0*rot(2.0*(time.elapsed * 0.03)) + 5.0*rot(0.25*3.14519*2.0*(time.elapsed * 0.03));
    let sc3 = 5.0*rot(1.3547*(time.elapsed * 0.03)) + 5.0*rot(2.7547*(time.elapsed * 0.03));
    let sc4 = custom.Pd*rot(0.4*(time.elapsed * 0.03) + t);
...
fn point_gen() {
    ...
    let sc = custom.Pe*rot(0.5*(time.elapsed * 0.03));
    let delta = 0.25*vec3(0.1+sc.x,0.2,0.4+sc.y)*sin(50.0*vec3(1.1+sc.y,1.0,1.3)*r4.x) - 0.0*vec3(0.0,1.0,0.0);
    p += 2.0*center * delta;
    p += vec3(0.0,0.0,0.0);

    let t = custom.Time;// mix(r4.y, 1.0, 0.3);
    var color = mix(vec3(1.0,0.4,0.1), vec3(0.10, 0.40, 1.0), r4.x);
